...
namespace day2 {
    class Program {
        static void Main(string[] args) {
            int result = 0;
            List<string> numList = @"515	912	619	2043	96	93	2242	1385	2110	860	2255	621	1480	118	1230	99
...
707	668	1778	1687	2073	1892	62	1139	908	78	1885	800	945	712	57	65".Replace("\r", null).Replace("\t", " ").Split('\n').ToList();

            foreach (var row in numList) {
                List<int> dings2 = row.Split(' ').Select(x => int.Parse(x)).ToList();
                dings2.Sort();
                result += dings2.Last() - dings2.First();
            }
            ...
            foreach (var row in numList) {
                List<int> dings2 = row.Split(' ').Select(x => int.Parse(x)).ToList();
                for (int i = 0; i < dings2.Count(); i++) {
                    for (int j = 0; j < dings2.Count(); j++) {
                        if (!dings2[i].Equals(dings2[j])) {
                            double res = (double)dings2[i] / dings2[j];
